...
function editClient(data) {
    ...
    if (data.picture != null && data.picture != "") {
        $("#img-change-profile2").attr(
            "src",
            routePublicStorage + "clients/" + data.picture
        );
    } else {
        $("#img-change-profile2").attr(
            "src",
            routePublicImages + "user_default.png"
        );
    }
    ...
    $("#recipient-date-of-birth-edit").val(
...
function showHiddenFields(tableId, btn) {
    showOverlay();
    $.ajax({
        url: "https://pilatessg.classerp.es/administration_config/check_status_hide_attr",
        type: "POST",
        data: {
